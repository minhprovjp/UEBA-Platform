...
def parse_and_append_log_data(new_lines, sessions):
    ...
    for line_content in new_lines:
        ...
        if connect_match:
            ...
            # === LỌC BỎ CÁC LỆNH SET TẠI ĐÂY ===
            # Chuyển về chữ hoa và kiểm tra xem có bắt đầu bằng các chuỗi này không
            normalized_query = query_text.strip().upper()
            if normalized_query.startswith('SET NAMES') or normalized_query.startswith('SET SESSION AUTOCOMMIT') or normalized_query.startswith('SET AUTOCOMMIT'):
                continue # Bỏ qua dòng log này và đi đến dòng tiếp theo
            # ======================================
            
            last_query_timestamp = data['timestamp']
            last_query_thread_id = data['thread_id']
            current_multiline_query_parts.append(query_text)
        elif quit_match:
